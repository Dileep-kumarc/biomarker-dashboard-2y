...
export function CustomLineChart({
  ...
  const chartConfig = useMemo(() => {
    ...
    if (clinicalRange) {
      // Use clinical range with some padding
      ...
      minValue = Math.max(0, dataMin - valueRange * 0.3)
      maxValue = dataMax + valueRange * 0.3
    }
  ...
  // Generate Y-axis ticks with better spacing
  const yTicks = []
  const tickCount = 8
