...
export function BiomarkerDashboard() {
  ...
  // Remove the export and settings state variables:
  ...
  //       currentValues: {
  //         totalCholesterol: latestData.totalCholesterol,
  //         hdlCholesterol: latestData.hdlCholesterol,
  //         ldlCholesterol: latestData.ldlCholesterol,
  //         triglycerides: latestData.triglycerides,
  //         creatinine: latestData.creatinine,
  //         vitaminD: latestData.vitaminD,
  //         vitaminB12: latestData.vitaminB12,
  //         hba1c: latestData.hba1c,
  //       },
  //       clinicalSummary: {
  //         optimalResults: [
  //           "Excellent Glucose Control (HbA1c: 5.8%)",
  //           "Optimal Total Cholesterol (136 mg/dL)",
  //           "Stable Kidney Function (Creatinine: 1.18 mg/dL)",
  //         ],
  //         areasForImprovement: [
  //           "Vitamin D Deficiency (18.73 ng/mL) - Critical",
  //           "Low HDL Cholesterol (36 mg/dL)",
  //           "Fluctuating Triglycerides (177 mg/dL)",
  //         ],
  //       },
  //     }
  ...
  //       link.href = url
  //       link.download = `biomarker-report-${new Date().toISOString().split("T")[0]}.json`
  //       link.click()
  //       URL.revokeObjectURL(url)
  //     } else if (exportFormat === "csv") {
  //       const csvData = biomarkerData.map((item) => ({
  //         Date: item.date,
  //         "Total Cholesterol": item.totalCholesterol,
  //         "HDL Cholesterol": item.hdlCholesterol,
  //         "LDL Cholesterol": item.ldlCholesterol,
  ...
  //       const link = document.createElement("a")
  //       link.href = url
  //       link.download = `biomarker-data-${new Date().toISOString().split("T")[0]}.csv`
  //       link.click()
  //       URL.revokeObjectURL(url)
  //       URL.revokeObjectURL(url)
  ...
  //       const reportText = `
  // ECOTOWN HEALTH ANALYTICS - BIOMARKER REPORT
  // Generated: ${new Date().toLocaleDateString()}

  // PATIENT INFORMATION:
  // Name: MR. MANJUNATH SWAMY
  // Age/Gender: 56Y/M
  // Monitoring Period: Nov 2023 - Apr 2025
  // Total Data Points: ${biomarkerData.length}

  // CURRENT BIOMARKER VALUES:
  // • Total Cholesterol: ${latestData.totalCholesterol} mg/dL
  // • HDL Cholesterol: ${latestData.hdlCholesterol} mg/dL
  // • LDL Cholesterol: ${latestData.ldlCholesterol} mg/dL
  // • Triglycerides: ${latestData.triglycerides} mg/dL
  // • Creatinine: ${latestData.creatinine} mg/dL
  // • Vitamin D: ${latestData.vitaminD} ng/mL
  // • Vitamin B12: ${latestData.vitaminB12} pg/mL
  // • HbA1c: ${latestData.hba1c}%
  ...
  // RECOMMENDATIONS:
  // 1. Immediate vitamin D supplementation (2000-4000 IU daily)
  // 2. Increase aerobic exercise (150 min/week) for HDL improvement
  // 3. Reduce refined carbohydrates for triglyceride control
  // 4. Follow-up testing in 3 months

  // This report is for healthcare professional use and patient education.
  ...
  //       const link = document.createElement("a")
  //       link.href = url
  //       link.download = `biomarker-report-${new Date().toISOString().split("T")[0]}.txt`
  //       link.click()
  //       URL.revokeObjectURL(url)
  //     }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/30 to-gray-50">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Remove both modal sections at the bottom: */}
        {/* Export Modal */}
        {/* {showExportModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
              <h3 className="text-lg font-semibold mb-4">Export Biomarker Report</h3>
              ...
              <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center">
                      <input
                        ...
                        className="mr-2"
                      />
                      <span className="text-sm">PDF Report (Comprehensive)</span>
                    </label>
                    <label className="flex items-center">
                      <input
                        type="radio"
                        ...
                        className="mr-2"
                      />
                      <span className="text-sm">CSV Data (Spreadsheet)</span>
                    </label>
                    <label className="flex items-center">
                      <input
                        type="radio"
                        ...
                        className="mr-2"
                      />
                      <span className="text-sm">JSON Data (Raw)</span>
                    </label>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 rounded-lg p-3">
                  <p className="text-xs text-gray-600">
                    {exportFormat === "pdf" && "Comprehensive report with clinical insights and recommendations"}
                    {exportFormat === "csv" && "Raw biomarker data suitable for spreadsheet analysis"}
                    {exportFormat === "json" && "Complete dataset including metadata and clinical summaries"}
                  </p>
                </div>
              </div>
            ...
            <div className="bg-white rounded-lg p-6 w-full max-w-lg mx-4">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="space-y-3">
                    <label className="flex items-center justify-between">
                      <span className="text-sm text-gray-700">Show clinical ranges on charts</span>
                      <input type="checkbox" defaultChecked className="rounded" />
                    </label>
                    <label className="flex items-center justify-between">
                      <span className="text-sm text-gray-700">Enable trend indicators</span>
                      <input type="checkbox" defaultChecked className="rounded" />
                    </label>
                    <label className="flex items-center justify-between">
                      <span className="text-sm text-gray-700">Show percentage changes</span>
                      <input type="checkbox" defaultChecked className="rounded" />
                    </label>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <label className="flex items-center justify-between">
                      <span className="text-sm text-gray-700">Alert for critical values</span>
                      <input type="checkbox" defaultChecked className="rounded" />
                    </label>
                    <label className="flex items-center justify-between">
                      <span className="text-sm text-gray-700">Monthly summary reports</span>
                ...
                <div className="bg-blue-50 rounded-lg p-4">
                  <h5 className="font-medium text-blue-800 mb-2">About EcoTown Health Analytics</h5>
                  <p className="text-xs text-blue-700">
                    Version 2.1.0 - Advanced biomarker intelligence platform with predictive analytics and clinical
                    decision support. Built for healthcare professionals and patient education.
                  </p>
                </div>
              </div>
